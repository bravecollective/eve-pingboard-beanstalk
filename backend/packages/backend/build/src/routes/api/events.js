...
function getRouter(options) {
    ...
    router.post('/', user_roles_1.userRoles.requireOneOf(user_roles_1.UserRoles.EVENTS_WRITE), async (ctx) => {
    ...
    router.put('/:eventId', user_roles_1.userRoles.requireOneOf(user_roles_1.UserRoles.EVENTS_WRITE), async (ctx) => {
    ...
    router.delete('/:eventId', user_roles_1.userRoles.requireOneOf(user_roles_1.UserRoles.EVENTS_WRITE), async (ctx) => {
        const eventId = parseInt(ctx.params['eventId']);
        if (!Number.isFinite(eventId) || eventId < 0) {
            throw new http_errors_1.BadRequest();
        }
